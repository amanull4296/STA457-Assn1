...
dataDir <- "~/GitHub/STA457-Assn1/Assn 1/DJ Data Monthly"
...
ea_v <- ea_volatility(dlogrets)
dj_tsmom_performance <- tsmom_performance(mlogrets, dlogrets)
...
ea_volatility_monthly <- function(retX_daily) {
  #need to solve for delta
  #according to paper delta has been solved for as
  delta <- 60/61
  #number of stocks
  ...
  #number of periods provided
  res <- matrix()
  for (t in seq(261, dim(retX_daily)[2], 260)){
...
predictiveRegression <- function(retX, retX_daily){
  num_stocks <- 30
  #number of periods of data provided
  months <- dim(retX)[2]
  optimal_h <- c()
  volatility <- ea_volatility(retX_daily)
...
monthly <- ea_volatility_monthly(dlogrets)
asdffasd <- predictiveRegression(mlogrets, dlogrets)