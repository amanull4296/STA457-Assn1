...
#code for having this in a table
dj_tsmom_performance_table <- t(rbind(round((dj_tsmom_performance$return)/10,4),  round((dj_tsmom_performance$volatility),4), round(((dj_tsmom_performance$sharpe)/10),4)))

colnames(dj_tsmom_performance_table)[1:3] <- c("Annualized expected return",  "Annualized volatility","Annualized sharpe ratio")

#code for plots...
par(mfrow = c(2,2))
plot(c(61:dim(mlogrets)[2]), dj_tsmom_performance$cumul_return, main = "cumulative log returns", type ="l", xlab = "month", ylab="cumulative return", ylim = c(0, 0.8))
...
plot(seq(61,dim(mlogrets)[2],12), dj_tsmom_performance$ea_volatilities, main = "annualized ex ante volatilities", type = 'l', xlab = "month", ylab="annualized ex ante volatility", ylim = c(0, 0.04))

plot(c(61:dim(mlogrets)[2]), dj_tsmom_performance$sharpes, main = "sharpe ratio", type = 'l', xlab = "month", ylab="sharpe ratio", ylim = c(0, 10))

 
#kable
kable(dj_tsmom_performance_table, caption = "EW performance metrics")
